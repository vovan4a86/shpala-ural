...
class AdminGalleryController extends AdminController {
	...
	{
		$g = Gallery::find(4); //Фотогал
		$g->params = ['thumbs' => [
			1 => '225x222', //admin list
		], 'fields' => [
	...
	public function postImageUpload($gallery_id)
	{
		...
		if ($images) foreach ($images as $image) {
			...
			$image = Image::make(base_path() . GalleryItem::UPLOAD_PATH . $file_name)
				->resize(1920, 1080, function ($constraint) {
				    $constraint->aspectRatio();
				    $constraint->upsize();
				});
			if(in_array($gallery_id, [3,4])){ //фото наших отелей, фотогалерея
