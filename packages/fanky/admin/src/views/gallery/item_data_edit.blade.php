<form action="{{ route('admin.gallery.imageDataSave', [$image->id]) }}" onsubmit="galleryImageDataSave(this, event)" style="width:600px;">
	...
	@foreach ($image->gallery->params['fields'] as $field => $params)
		<div class="form-group">
			...
			@if ($params['type'] == 0)
				<input id="gallery-image-{{ $field }}" class="form-control" type="text" name="{{ $field }}" value="{{ $image->data[$field] or '' }}">
			@elseif ($params['type'] == 1)
				<textarea id="gallery-image-{{ $field }}" class="form-control" name="{{ $field }}" rows="4">{{ $image->data[$field] or '' }}</textarea>
			@endif
		</div>
	@endforeach
