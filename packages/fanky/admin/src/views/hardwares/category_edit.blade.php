...
@section('page_name')
    <h1>{{ $item->id? $item->name: 'Новая категория' }}</h1>
@stop
...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li class="active">{{ $item->id? $item->name: 'Новая категория' }}</li>
    </ol>
@stop
...
@section('content')
    <div class="row">
        ...
        <div id="catalog-content" class="col-md-9">
            <div class="box box-solid">
                <div class="box-body">
                    <form action="{{ route('admin.hardware.category-save') }}" onsubmit="categorySave(this, event)">
                        @if($item->id)
                        ...
                        <div class="nav-tabs-custom">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_1">
                                    ...
                                    {!! Form::groupSelect('parent_id', [0=>'--Корень--'] + $categories->pluck('name', 'id')->all(), $item->parent_id, 'Категория') !!}
                                    {!! Form::groupText('h1', $item->h1, 'H1') !!}

                                    <div class="form-group">
                                        <label for="article-image">Изображение</label>
                                        <input id="article-image" type="file" name="image" value="" onchange="return imageAttache(this, event)">
                                        <div id="article-image-block">
                                            @if ($item->image)
                                                <img class="img-polaroid" src="{{ $item->thumb(1) }}" height="100" data-image="{{ $item->image_src }}" onclick="return popupImage($(this).data('image'))">
                                            @else
